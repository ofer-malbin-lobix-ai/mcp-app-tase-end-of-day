...
import styles from "./last-update-end-of-day-widget.module.css";
...
function LastUpdateApp() {
  ...
  const { app, error } = useApp({
    appInfo: { name: "Last Update", version: "1.0.0" },
    capabilities: {},
    onAppCreated: (app) => {
  ...
  useEffect(() => {
    ...
    try {
      app.callServerTool({ name: "get-last-update-data", arguments: {} })
        .then((result) => {
...
function LastUpdateAppInner({
  ...
  return (
    <WidgetLayout title="Last Update" app={app} hostContext={hostContext}>
