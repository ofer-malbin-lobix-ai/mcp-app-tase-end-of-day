...
import styles from "./my-position-end-of-day-widget.module.css";
...
function extractEndOfDaySymbolsData(callToolResult: CallToolResult | null | undefined): EndOfDaySymbolsData | null {
  try {
    ...
    // Fallback to text content (from get-my-position-end-of-day-data or app.callServerTool)
...
function EndOfDaySymbolsAppInner({
  ...
  const handleRefresh = useCallback(async (symbols?: string[], dateFrom?: string, dateTo?: string) => {
    ...
    try {
      console.info("Calling get-my-position-end-of-day-data tool...", { symbols, dateFrom, dateTo });
      ...
      const result = await app.callServerTool({
        name: "get-my-position-end-of-day-data",
        arguments: args,
      });
      const data = extractEndOfDaySymbolsData(result);
      if (data) {
  ...
  const subtitle = data
    ? `${data.symbols?.length > 0 ? data.symbols.join(", ") : "All symbols"}${data.dateFrom ? ` · ${data.dateFrom}` : ""}${data.dateTo && data.dateTo !== data.dateFrom ? ` — ${data.dateTo}` : ""}`
    : undefined;

  return (
    <WidgetLayout
      title="My Position End of Day"
      subtitle={subtitle}
      app={app}
      hostContext={hostContext}
    >
